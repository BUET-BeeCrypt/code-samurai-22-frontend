...
import { getAllProjects, getAllProposals } from "../api";

export default function PropVisualization() {
  ...
  useEffect(() => {
    ...
    if (filter.dateUpto) filtered = filtered.filter(proj => {
      const projEnd = new Date(Date.parse(proj.proposal_date.substr(0, 10)));
